    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for position in newPosDist.keys():
                distribution[position] += self.beliefs[oldPos] * newPosDist[position]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        number = self.numParticles / len(samples)
        while number > 0:
            self.particles.extend(samples)
            number -= 1
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                addedprob = addedprob * prob
            distribution[particle] += addedprob
        distribution.normalize()
        particleNum = self.numParticles
        resample = []
        while particleNum > 0:
